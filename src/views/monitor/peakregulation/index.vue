<template>
  ...
  <script>
  import { getUsers, peakregulation_ExcelToSql, peakregulation_getUserData } from '@/api//monitor/peakregulation'
  ...
  export default {
    ...
    methods: {
      ...
      submitExcelToSql() {
        ...
        peakregulation_ExcelToSql(formData).then(res => { // 调用接口
          this.uploading = false
          if (res.code === 200) {
      ...
      getData() {
        peakregulation_getUserData(this.form).then(res => {
          console.log(res)
          this.tableData = res.data
          this.total = res.total
        }).catch(err => {
