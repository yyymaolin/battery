<template>
  ...
  <script>
  import { getUsers, recycle_ExcelToSql, recycle_getUserData } from '@/api/monitor/recycle'
  ...
  export default {
    ...
    methods: {
      // 获取用户列表/搜索功能
      search() {
        recycle_getUserData(this.form).then(res => {
          console.log(res)
      ...
      submitExcelToSql() {
        ...
        recycle_ExcelToSql(formData).then(res => { // 调用接口
          this.uploading = false
          if (res.code === 200) {
