<template>
      ...
      <el-row>
        <el-col>
          ...
          >
            ...
            />
            <el-table-column
              prop="battery_standard_voltage"
              label="电芯标称电压"
              width="130"
            />
            <el-table-column
              prop="energy_storage_unit_capacity"
              label="储能单元容量"
              width="180"
            />
            <el-table-column
              prop="energy_storage_unit_voltage"
              label="储能单元电压"
              width="180"
            />
            <el-table-column
  ...
  <script>
  import { getUsers, powerstation_ExcelToSql, powerstation_getUserData } from '@/api/monitor/powerstation'
  export default {
    name: 'Users',
    data() {
      return {
    ...
    methods: {
      ...
      submitExcelToSql() {
        ...
        powerstation_ExcelToSql(formData).then(res => { // 调用接口
          this.uploading = false
          if (res.code === 200) {
      ...
      getData() {
        powerstation_getUserData(this.form).then(res => {
          console.log(res)
          this.tableData = res.data
          this.total = res.total
        }).catch(err => {
