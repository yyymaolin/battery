<template>
  ...
  <script>
  import { getUsers, running_ExcelToSql, running_getUserData } from '@/api/monitor/running'
  ...
  export default {
    ...
    methods: {
      // 获取用户列表/搜索功能
      search() {
        running_getUserData(this.form).then(res => {
          console.log(res)
      ...
      submitExcelToSql() {
        ...
        running_ExcelToSql(formData).then(res => { // 调用接口
          this.uploading = false
          if (res.code === 200) {
