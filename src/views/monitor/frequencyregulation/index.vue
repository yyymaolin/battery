<template>
      ...
      <el-row>
        <el-col>
          ...
          >
            ...
            />
            <el-table-column
              prop="frequency_coefficient"
              label="调频系数"
              width="180"
            />
            <el-table-column
              prop="frequency_subsidies"
              label="调频补贴"
              width="180"
            />
            <el-table-column
  ...
  <script>
  import { getUsers, frequencyregulation_ExcelToSql,  frequencyregulation_getUserData } from '@/api//monitor/frequencyregulation'
  ...
  export default {
    ...
    methods: {
      ...
      submitExcelToSql() {
        ...
        frequencyregulation_ExcelToSql(formData).then(res => { // 调用接口
          this.uploading = false
          if (res.code === 200) {
      ...
      getData() {
        frequencyregulation_getUserData(this.form).then(res => {
          console.log(res)
          this.tableData = res.data
          this.total = res.total
        }).catch(err => {
